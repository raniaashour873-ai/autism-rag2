...
def run_step3(chunks_path: str = "step2_chunks.json",
    ...
    print(f"\n✅ تم فهرسة {len(chunks)} chunk في: {db_path}")
    return collection
...
def test_query(collection, query_text: str, top_k: int = 3):
    ...
    for i, (doc, meta, distance) in enumerate(zip(
        ...
        print(f"\n  النتيجة {i+1} (المسافة: {distance:.4f}) — صفحة {meta['page_number']} | {meta['section_title']}")
        print(f"  {doc[:150]}")


if __name__ == "__main__":
    collection = run_step3()
